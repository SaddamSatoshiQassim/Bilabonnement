...
public class JdbcRentalAgreementRepository implements RentalAgreementRepository {
    ...
    public void save(RentalAgreement rentalAgreement) {
        ...
        String sql =
                "INSERT INTO rental_agreement(customer_id,car_id,start_date,end_date,rental_price,pickup_location,return_location) VALUES(?,?,?,?,?,?,?)";
        ...
        try(Connection connection = DriverManager.getConnection(url,user,password);
            ...
            statement.setInt(
                    1,
                    rentalAgreement.getCustomer().getId()
            );
            ...
            statement.setDate(
                    3,
                    Date.valueOf(rentalAgreement.getStartDate())
            );

            if(rentalAgreement.getEndDate()!=null) {
                statement.setDate(
                        4,
                        Date.valueOf(rentalAgreement.getEndDate())
                );
            } else {
                statement.setNull(
                        4,
            ...
            statement.setBigDecimal(
                    5,
                    rentalAgreement.getRentalPrice()
            );

            statement.setString(
                    6,
                    rentalAgreement.getPickupLocation().getName()
            );

            statement.setString(
                    7,
                    rentalAgreement.getReturnLocation().getName()
            );
    ...
    public void update(RentalAgreement rentalAgreement) {
        ...
        String sql =
                "UPDATE rental_agreement SET customer_id=?, car_id=?, start_date=?, end_date=?, rental_price=?, pickup_location=?, return_location=? WHERE rental_id=?";
    ...
    public void deleteById(int id) {
        ...
        String sql =
                "DELETE FROM rental_agreement WHERE rental_id=?";
